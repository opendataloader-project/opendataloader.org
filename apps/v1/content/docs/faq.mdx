...
| Feature | OpenDataLoader | Docling | MinerU |
|---------|----------------|---------|--------|
| Approach | Rule-based heuristics | Deep learning | Deep learning |
| GPU Required | No | Yes | Yes |
| Output Consistency | Deterministic | Variable | Variable |
| Bounding Boxes | All elements | Limited | Limited |
| AI Safety Filters | Built-in | No | No |
| Tagged PDF Support | Native | Limited | No |

OpenDataLoader uses deterministic rule-based heuristics instead of deep learning. This means: consistent output (same input = same output), no GPU required, faster processing, and no model hallucinations.
...
Enable the XY-Cut++ reading order algorithm:

```python
opendataloader_pdf.convert(
    input_path="two-column-paper.pdf",
    output_dir="output/",
    reading_order="xycut"  # Preserves correct reading order
)
```
...
OpenDataLoader processes **100+ pages per second** on typical hardware (CPU only, no GPU). Exact speed depends on:
- Document complexity
...
Yes. Point to a folder instead of a single file:

```python
opendataloader_pdf.convert(
    input_path="documents/",  # Folder containing PDFs
    output_dir="output/",
...
The [European Accessibility Act (EAA)](https://commission.europa.eu/strategy-and-policy/policies/justice-and-fundamental-rights/disability/union-equality-strategy-rights-persons-disabilities-2021-2030/european-accessibility-act_en) took effect June 28, 2025, requiring accessible digital documents across the EU. This means more PDFs will be properly tagged.
...
Try both detection methods by default. For complex tables, Table AI support is coming soon.
